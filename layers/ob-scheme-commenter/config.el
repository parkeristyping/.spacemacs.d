...
(defun wrap-it-before-you-tap-it (body)
  ;; Identify all ";=>"s
  ;; Wrap previous parens in (cons comment-results previous-parens)
  ;; Wrap whole body in (let (comment-results '()) body (reverse comment-results))
  (message body)
  body)
...
;          (let (value)
;          (dolist (body-part (split-string full-body ";=>") value)
;            (cons (org-babel-scheme-execute-with-geiser
;             body-part      ; code
;             (string= result-type "output") ; output?
;             impl         ; implementation
;             (and (not (string= session "none")) session)) value)
;            ))); session
...
; (let (value)
;   (message (cons 5 value)))
;
; (concat "hey" "you" "you")
; (replace-regexp-in-string "(.+) ;=>" "hey" "(+ 1 1)\n(+ 1 1) ;=>")
;
; (replace-regexp-in-string (regexp "\(.+\) ;=>") "yo" "(+ 5 5) (+ 1 1) ;=>")
;
; (dolist (thing '("1hey" "2you"))
;   (message thing))
;
; (regexp-builder)
;
